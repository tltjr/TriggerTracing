	...
	public static class TriggerTracing
	{
		...
		private static void OnTraceEnabledChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
		{
			...
			else
			{
				...
				{
					foreach (BeginStoryboard bsb in actionCollection
						.Select(triggerAction => triggerAction as BeginStoryboard)
						.Where(bsb => bsb != null && bsb.Storyboard != null && bsb.Storyboard is TriggerTraceStoryboard))
					{
			...
			public override void TraceEvent(TraceEventCache eventCache, string source, TraceEventType eventType, int id, string format, params object[] args)
			{
				...
				if (format.StartsWith("Storyboard has begun;"))
				{
					...
					if (storyboard != null)
					{
						...
						Debug.WriteLine("Element: {0}, {1}: {2}: {3}",
							targetElement,
